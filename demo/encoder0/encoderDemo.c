...
//	h264Param.nCodingMode = VENC_FIELD_CODING;
	...
	logi("1. Video Encoder Create\n");
	
	pVideoEnc = VideoEncCreate(codecType);
	if(!pVideoEnc)
		loge("pVideoEnc is NULL.");

	logd("AFTER VideoEncCreate.\n");
	...
	logi("2. Video Encoder Init\n");
	
	ret = VideoEncInit(pVideoEnc, &baseConfig);
	if(ret)
		loge("VideoEncInit\n");
	
	logd("after VideoEncInit\n");
	...
	logi("3. Alloc Input Buffer with using 'vencoder'\n");

	ret = AllocInputBuffer(pVideoEnc, &bufferParam);
	if(ret)
		loge("AllocInputBuffer\n");

	logd("after AllocInputBuffer\n");
	...
	while(testNumber > 0)
	{
		...
		logi("4. Get One Alloc Input Buffer from 'vencoder'\n");
		ret = GetOneAllocInputBuffer(pVideoEnc, &inputBuffer);
		if(ret)
			loge("GetOneAllocInputBuffer\n");
		...
		logi("5. Flush Cache for Alloc Input Buffer in order to keep data accordance\n");
		ret = FlushCacheAllocInputBuffer(pVideoEnc, &inputBuffer);
		if(ret)
			loge("FlushCacheAllocInputBuffer\n");

		logi("6. Add One Input Buffer which is alloced from 'vencoder'\n");
		ret = AddOneInputBuffer(pVideoEnc, &inputBuffer);
		if(ret)
			loge("AddOneInputBuffer\n");

		logi("7. Video Encode One Frame\n");
		ret = VideoEncodeOneFrame(pVideoEnc);
		
		logd("VideoEncodeOneFrame ret: %d\n", ret);
		switch(ret)
		{
		case -1:
			loge("Encode error.\n");
		...
		case 0:
			...
			break;
		case 1:
			loge("Can't get input frame.\n");
			break;
		case 2:
			logw("Output stream buffer is overflow.\n");
			break;
		default:
			logw("Unknown encoder state.\n");
			break;
		}
		
		logi("8. Get an Already Used Input Buffer from encoder");
		...
		logd("size: %d,%d", outputBuffer.nSize0,outputBuffer.nSize1);
